...
window.getSafeImageHTML = function(id, hint, extraAttrs = '') {
    ...
    renderPlayInventory(invType) {
        ...
        if (items.length === 0) {
            panel.innerHTML = '<div style="width:100%; text-align:center; margin-top:20px; color:rgba(0,0,0,0.5);">La mochila est� vac�a.</div>';
            return;
        }
    ...
    bindEvents() {
        ...
        window.addEventListener('dragenter', e => {
            ...
            if (!e.dataTransfer || !e.dataTransfer.types || !Array.from(e.dataTransfer.types).includes('Files') || Array.from(e.dataTransfer.types).includes('application/json')) return;
            dragDepth++;
