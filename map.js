...
class IsometricMap {
    ...
    getWallIsoCoords(wx, wy, flipped, bbox, floorNum = 0) {
        ...
        // Paredes traseras: 
        // Si flipped == true (Pared Derecha), avanza a lo largo de x (var�a wx), y se ancla al fondo en box.ymin
        // Si flipped == false (Pared Izquierda), avanza a lo largo de y (var�a wx), y se ancla al fondo en box.xmin
    ...
    _hitTestPlayFloors(screenX, screenY) {
        ...
        // Iteramos de arriba hacia abajo (Grp 2 -> 1 -> 0) para chequear superposici�n visual
        for (let g = 2; g >= 0; g--) {
            const surf = window.mapsAtlas.find(s => s.kind === 'floor' && s.groupNum === g);
            if (surf) {
                const off = this.getFloorOffset(g);
                // Inversi�n geom�trica isom�trica
